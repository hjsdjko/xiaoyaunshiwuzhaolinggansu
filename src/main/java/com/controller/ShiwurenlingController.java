        ...
        if(shiwurenling !=null){
            ...
            BeanUtils.copyProperties( shiwurenling , view );//把实体数据重构到view中
                ...
                if(shiwuzhaoling != null){
                    BeanUtils.copyProperties( shiwuzhaoling , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setShiwuzhaolingId(shiwuzhaoling.getId());
                    view.setShiwuzhaolingYonghuId(shiwuzhaoling.getYonghuId());
                }
                //级联表
...
//                            shiwurenlingEntity.setShiwurenlingYesnoTypes(Integer.valueOf(data.get(0)));   //审核 要改的
//                            shiwurenlingEntity.setShiwurenlingYesnoText(data.get(0));                    //详情 要改的
