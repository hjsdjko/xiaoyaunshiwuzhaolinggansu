    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(wupinguashiLiuyan !=null){
            ...
            BeanUtils.copyProperties( wupinguashiLiuyan , view );//把实体数据重构到view中
                ...
                if(wupinguashi != null){
                    BeanUtils.copyProperties( wupinguashi , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setWupinguashiId(wupinguashi.getId());
                    view.setWupinguashiYonghuId(wupinguashi.getYonghuId());
                }
                //级联表
...
//            wupinguashiLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
    ...
    */
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<WupinguashiLiuyanEntity> wupinguashiLiuyanList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            WupinguashiLiuyanEntity wupinguashiLiuyanEntity = new WupinguashiLiuyanEntity();
//                            wupinguashiLiuyanEntity.setWupinguashiId(Integer.valueOf(data.get(0)));   //物品 要改的
//                            wupinguashiLiuyanEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            wupinguashiLiuyanEntity.setWupinguashiLiuyanText(data.get(0));                    //留言内容 要改的
//                            wupinguashiLiuyanEntity.setReplyText(data.get(0));                    //回复内容 要改的
//                            wupinguashiLiuyanEntity.setInsertTime(date);//时间
//                            wupinguashiLiuyanEntity.setUpdateTime(sdf.parse(data.get(0)));          //回复时间 要改的
//                            wupinguashiLiuyanEntity.setCreateTime(date);//时间
                            wupinguashiLiuyanList.add(wupinguashiLiuyanEntity);
                        ...
                        //查询是否重复
                        wupinguashiLiuyanService.insertBatch(wupinguashiLiuyanList);
                        return R.ok();
                    }
                }
            }
